...
const VoucherBetwayRedeem: React.FC<VoucherBetwayRedeemProps> = ({ onBack, skipCheckStatus, prefillPin, prefillAmount }) => {
  ...
  const handleRedeem = async () => {
    ...
    setStep('redeeming');
    try {
      const result = await redeemBetway({
      ...
      setError(err.message || 'Failed to redeem to Betway');
      setStep('error');
    } finally {
      setLoading(false);
    }
        ...
        <CardContent className="p-8">
          {step === 'form' && (
            <form onSubmit={handleCheckVoucher} className="space-y-6">
              <h2 className="text-2xl font-bold mb-2 text-center">Redeem Voucher to Betway</h2>
              <p className="text-sm text-gray-600 text-center mb-4">
                Top up your Betway account using your cell phone number
              </p>
              <Input
              ...
              />
              <Input
                placeholder="Betway Account Number (Cell Phone e.g. 0830000000)"
                value={mobile}
              ...
              <div className="flex gap-2">
                ...
                <Button type="submit" variant="default" className="w-1/2" disabled={loading}>
          ...
          {step === 'quote' && (
            <div className="space-y-6">
              ...
              <div className="text-center">
                ...
                <div className="text-sm text-gray-500">Betway Account: {mobile}</div>
                {!skipCheckStatus && <div className="text-sm text-gray-500">Status: {voucherStatus}</div>}
              </div>
              <div className="text-center text-sm text-gray-600">Proceed to top up your Betway account?</div>
              {error && <div className="text-red-500 text-sm text-center">{error}</div>}
              <div className="flex gap-2">
                <Button variant="outline" onClick={handleReset} className="w-1/2">Cancel</Button>
                <Button variant="default" onClick={handleRedeem} className="w-1/2" disabled={loading || (!skipCheckStatus && voucherStatus !== 'Active')}>
                  {loading ? 'Processing...' : 'Top Up Betway'}
                </Button>
              </div>
            </div>
          )}
...
export default VoucherBetwayRedeem;